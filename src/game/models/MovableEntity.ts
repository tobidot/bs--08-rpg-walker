...
export class MoveableEntity extends Entity {
    ...
    public seconds_to_attack: number = 0;
    ...
    public update(delta_seconds: number) {
        ...
        this.entities_aware_of = this.game.model.physics.pickWithinRect(rect)
            ...
            .filter((other): other is Entity => other instanceof Entity && other !== this && !other.is_hidden);
        this.entities_colliding_with = this.entities_aware_of
            .filter(other => other.hit_box.intersects(this.hit_box));
        // attack
        this.seconds_to_attack = Math.max(0, this.seconds_to_attack - delta_seconds);
        if (this.attack_damage > 0 && this.seconds_to_attack <= 0) {
            const attackable = this.entities_aware_of.filter(this.isAttackable);
            while (this.seconds_to_attack <= 0 && attackable.length > 0) {
        ...
        this.entities_colliding_with.filter(this.isBouncable).forEach((other: Entity) => {
            ...
            if (other.is_wood && this.harvest_wood > 0 && this.seconds_to_attack <= 0) {
                other.wood -= this.harvest_wood;
                this.game.model.player.money += this.harvest_wood;
                this.seconds_to_attack = this.attack_delay_seconds;
            }
        });
    }

    public attack(target: Entity) {
        this.seconds_to_attack += Math.max(0.01, this.attack_delay_seconds);
